...
//creates SimpleXMLElement object
$xml_class = new SimpleXMLElement("<?xml version=\"1.0\"?><class></class>");

//function call to convert array to xml
array_to_xml($class,$xml_class);

//saves generated xml file

$xml_class->asXML('class.xml');

//function definition to convert array to xml

function array_to_xml($class, &$xml_class) {
    foreach($class as $key => $value) {
        if(is_array($value)) {
            if(!is_numeric($key)){
                $subnode = $xml_class->addChild("$key");
                array_to_xml($value, $subnode);
            }
            else {
                array_to_xml($value, $xml_class);
            }
        }
        else {
            $xml_class->addChild("$key", "$value");
        }
    }
}
?>
